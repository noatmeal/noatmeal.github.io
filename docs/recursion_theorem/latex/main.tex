...
\begin{definition}
  Let \( J \) be a set who's existence is guaranteed by 
...
\begin{example}
  ...
  \begin{itemize}
    ...
    \item \(3^{+} = 4\)
  \end{itemize}
  and we'll define two more compact names to be used in an informative example 
...
\begin{proof}
  ...
  \( n \in K \). 
  Recall that \( n \in J \) from \autoref{def:natural_numbers}. By defintion, 
  \( n^{+} \in J \). So we conclude from these two facts that 
  \( n^{+} \in \mathbb{N} \).
\end{proof}
...
\begin{proof}
  ...
  we are given that \( n \in S \Rightarrow n \in \mathbb{N} \). 
  Now suppose that \( n \in \mathbb{N} \). We can see that \( S \) is an 
...
\begin{proof}
  \( n^{+} \) is nonempty and 1 is empty. So we conclude that 
  \( n^{+} \neq 1 \).
\end{proof}
...
\begin{proof}
  ...
  Therefore, \( n^{+} \in S \) and so \( S = \mathbb{N} \).
\end{proof}
...
\begin{proof}
  ...
  \( m \subseteq n \) and \( n \subseteq m \) and so \( m = n \), a 
  contradiction. 
\end{proof}
